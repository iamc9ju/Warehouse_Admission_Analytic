...
SOURCES = [
    SourceWorkbook(
        academic_year=2568,
        tcas_round_code="TCAS1",
        tcas_round_name="Portfolio",
        path=Path("/Users/iamc9ju/Downloads/รอบ 1 ปี68.xlsx"),
    ),
    SourceWorkbook(
        academic_year=2568,
        tcas_round_code="TCAS1",
        tcas_round_name="Portfolio",
        path=Path("/Users/iamc9ju/Downloads/รอบ 1 ปี68(1).xlsx"),
    ),
    SourceWorkbook(
        academic_year=2568,
        tcas_round_code="TCAS2",
        tcas_round_name="Quota",
        path=Path("/Users/iamc9ju/Downloads/รอบ 2 ปี68.xlsx"),
    ),
    SourceWorkbook(
        academic_year=2568,
        tcas_round_code="TCAS2",
        tcas_round_name="Quota",
        path=Path("/Users/iamc9ju/Downloads/รอบ 2 ปี68(2).xlsx"),
    ),
    SourceWorkbook(
        academic_year=2568,
    ...
    SourceWorkbook(
        academic_year=2568,
        tcas_round_code="TCAS4",
        tcas_round_name="Direct Admission",
        path=Path("/Users/iamc9ju/Downloads/รอบ 4 ปี68.xlsx"),
    ),
    SourceWorkbook(
        academic_year=2569,
        tcas_round_code="TCAS1",
        tcas_round_name="Portfolio",
        path=Path("/Users/iamc9ju/Downloads/รอบ1 ปี69.xlsx"),
    ),
    SourceWorkbook(
        academic_year=2569,
        tcas_round_code="TCAS2",
        tcas_round_name="Quota",
        path=Path("/Users/iamc9ju/Downloads/รอบ 2 ปี69.xlsx"),
    ),
    SourceWorkbook(
        academic_year=2569,
        tcas_round_code="TCAS2",
        tcas_round_name="Quota",
        path=Path("/Users/iamc9ju/Downloads/รอบ 2 ปี69(2).xlsx"),
    ),
    SourceWorkbook(
        academic_year=2569,
        tcas_round_code="TCAS3",
    ...
    SourceWorkbook(
        ...
        tcas_round_code="TCAS4",
        tcas_round_name="Direct Admission",
        path=Path("/Users/iamc9ju/Downloads/รอบ 4 ปี69.xlsx"),
    ),
]
...
def build_data_quality_summary(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for key_values, source_df in df.groupby(keys, dropna=False):
        academic_year, tcas_round_code, tcas_round_name, source_file = key_values
        rows.append(
            {
                "academic_year": academic_year,
                "tcas_round_code": tcas_round_code,
                "tcas_round_name": tcas_round_name,
                "source_file": source_file,
                "source_rows": len(source_df),
                "unique_applicants": source_df["citizen_id"].nunique(),
                "duplicate_applicant_rows": len(source_df) - source_df["citizen_id"].nunique(),
                "missing_score_rows": int(source_df["score"].isna().sum()),
                "missing_priority_rows": int(source_df["priority"].isna().sum()),
                "missing_major_rows": int(source_df["major_id"].isna().sum()),
                "pii_columns_removed": ", ".join(sorted(PII_COLUMNS.intersection(source_df.columns))),
            }
        )
...
def build_year_overview(df: pd.DataFrame) -> pd.DataFrame:
    keys = ["academic_year"]
    grouped = df.groupby(keys, dropna=False)
    output = grouped.agg(
        application_choices=("citizen_id", "size"),
        ...
        unique_majors=("major_id", "nunique"),
        tcas_rounds=("tcas_round_code", "nunique"),
        source_files=("source_file", "nunique"),
        avg_score=("score", "mean"),
    )
    output = add_status_counts(grouped, output)
...
def format_numeric_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    count_columns = [
        ...
        or column in {"application_choices", "unique_majors", "source_files", "tcas_rounds"}
    ]
...
def main() -> None:
    ...
    round_summary = build_round_summary(combined)
    status_summary = build_status_summary(combined)
    data_quality = build_data_quality_summary(combined)
    round_overview = build_round_overview(combined)
    year_overview = build_year_overview(combined)

    write_csv(by_major, "admissions_all_rounds_2568_2569_by_major.csv")
    write_csv(round_summary, "admissions_all_rounds_2568_2569_summary.csv")
    write_csv(status_summary, "admissions_all_rounds_2568_2569_status_summary.csv")
    write_csv(data_quality, "admissions_all_rounds_2568_2569_data_quality.csv")
    write_csv(round_overview, "admissions_all_rounds_2568_2569_round_overview.csv")
    write_csv(year_overview, "admissions_all_rounds_2568_2569_year_overview.csv")

    print("Created all-round aggregate admissions files:")
    for path in sorted(OUTPUT_DIR.glob("admissions_all_rounds_2568_2569*.csv")):
        print(f"- {path}")
