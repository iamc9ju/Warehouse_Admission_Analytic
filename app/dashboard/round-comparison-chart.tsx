            ...
            {years.map((year, index) => {
              ...
              return <g key={year} onMouseLeave={() => setHovered(null)}>
                {metrics.map((metric) => {
                  ...
                  return <rect key={metric.key} x={x - 42} y={450 - top} width="84" height={Math.max(0, top)} fill={dimmed ? "#e5ded3" : metric.color} stroke="#fdfbf8" strokeWidth="0.6" tabIndex={0} role="img" aria-label={`${code} ปี ${year} ${metric.label} ${number(valueOf(row, metric.key))} คน`} onMouseEnter={() => { setHovered(metric.key); }} onFocus={() => setHovered(metric.key)} onBlur={() => setHovered(null)} style={{ cursor: "default", transition: "fill 180ms ease" }}><title>{`${metric.label}: ${number(valueOf(row, metric.key))} คน`}</title></rect>;
                })}
