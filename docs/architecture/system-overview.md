...
Excel admissions files / owned GA4 aggregate reports
  ...
  -> warehouse query-result exports
...
- The dashboard loader first attempts a server-side Neon query when `DATABASE_URL` is configured.
- If the live Neon query is unavailable, the dashboard reads `app/data/generated/warehouse-dashboard-snapshot.json`, which is generated by `npm run data:build` from warehouse query-result exports.
- `npm run data:check-static` prevents dashboard data arrays from being embedded in route or component source.
